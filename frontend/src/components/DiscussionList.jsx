...
function DiscussionList({ discussions, fetchDiscussions }) {
  ...
  const handleLike = async (discussionId) => {
    try {
      ...
      setLikedStatus({ ...likedStatus, [discussionId]: response.data.data.liked });
      fetchDiscussions();
    } catch (error) {
      console.error("Error toggling like:", error);
    }
  ...
  return (
    <div>
      {discussions.map((discussion) => (
        <div key={discussion._id} className="bg-[#151f2a] p-6 rounded-lg shadow-lg mb-4">
          <h2 
            className="font-bold text-lg text-white mb-2 cursor-pointer hover:underline"
          ...
          <div className="text-sm text-gray-500 mb-4">
            <p>Posted by: <span className="font-medium text-gray-200">{discussion.createdBy.username}</span></p>
            <p className="text-xs">{formatDate(discussion.createdAt)}</p>
          </div>
          {discussion.tags && discussion.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {discussion.tags.map((tag, index) => (
